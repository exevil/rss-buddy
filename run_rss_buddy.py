...
def parse_args():
    ...
    parser.add_argument("--api-key", required=True, help="OpenAI API key (required)")

    parser.add_argument(
        "--feeds", required=True, help="Comma-separated list of RSS feed URLs to process (required)"
    )
    ...
    parser.add_argument("--model", required=True, help="OpenAI model to use (required)")

    parser.add_argument(
        "--max-tokens", type=int, required=True, help="Maximum tokens for summaries (required)"
    )
