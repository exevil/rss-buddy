...
class TestFeedProcessor(unittest.TestCase):
    ...
    def test_alternate_date_fields(self):
        ...
        self.assertEqual(
            self.processor._parse_date(entry['published']).isoformat(),
            expected_date
        )
        ...
        self.assertEqual(
            self.processor._parse_date(entry['updated']).isoformat(),
            expected_date
        )
    ...
    def test_process_feed(self):
        ...
        with patch.object(self.processor, 'fetch_rss_feed', return_value=mock_feed):
            ...
            with patch.object(self.processor, 'is_recent', return_value=True):
                ...
                with patch.object(
                    self.processor, 'evaluate_article_preference',
                    return_value="FULL"
                ):
            ...
            with patch.object(self.processor, 'is_recent', return_value=True):
                ...
                with patch.object(
                    ...
                    with patch('xml.etree.ElementTree.ElementTree'):
                        ...
                        stored_data = {
                            'preference': 'FULL',
                            'date': self.test_entry['published']
                        }
    ...
    def test_process_feed_digest_updates(self):
        ...
        with patch.object(self.processor, 'fetch_rss_feed', return_value=mock_feed):
            ...
            with patch.object(self.processor, 'is_recent', return_value=True):
                ...
                with patch.object(
                    self.processor, 'evaluate_article_preference',
                    side_effect=get_preference
                ):
                    # Mock create_consolidated_summary to return our mock summary
                    with patch.object(
                        self.processor, 'create_consolidated_summary',
