...
# Example of using tzinfos in date parsing
# date_str = 'Tue, 01 Apr 2025 04:47:29 PDT'
            ...
            if file.endswith('.xml'):
                ...
                try:
                    ...
                    if channel is not None:
                        title = channel.find('title').text if channel.find('title') is not None and channel.find('title').text is not None else 'Untitled Feed'
                        link = channel.find('link').text if channel.find('link') is not None and channel.find('link').text is not None else '#'
                        description = channel.find('description').text if channel.find('description') is not None and channel.find('description').text is not None else 'No description'
                        last_build_date = channel.find('lastBuildDate').text if channel.find('lastBuildDate') is not None and channel.find('lastBuildDate').text is not None else datetime.now().strftime('%a, %d %b %Y %H:%M:%S GMT')
...
'''
                        ...
                        for item in items:
                            item_title = item.find('title').text if item.find('title') is not None and item.find('title').text is not None else 'Untitled Item'
                            item_link = item.find('link').text if item.find('link') is not None and item.find('link').text is not None else '#'
                            item_desc = item.find('description').text if item.find('description') is not None and item.find('description').text is not None else 'No description'
                            item_date = parser.parse(item.find('pubDate').text, tzinfos=TZINFOS).strftime('%a, %d %b %Y %H:%M:%S GMT') if item.find('pubDate') is not None and item.find('pubDate').text is not None else ''
